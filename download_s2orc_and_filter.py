...
if __name__ == '__main__':
    ...
    file2 = open("./data/download2.txt","r").read()
    temp2 = []
    for line2 in file2.split():
        if re.search(pattern, line2):
            temp2.append(re.search(pattern, line2).group("url"))
            
    download_linkss = []
    for i in range(100):
        download_linkss.append({"metadata":temp[i].strip("'"),"pdf_parses":temp2[i].strip("'")})


    METADATA_INPUT_DIR = 'metadata/raw/'
    METADATA_OUTPUT_DIR = 'metadata/cs/'
    PDF_PARSES_INPUT_DIR = 'pdf_parses/raw/'
    PDF_PARSES_OUTPUT_DIR = 'pdf_parses/cs/'
